...
export class ApiService {
  ...
  public updatedRedemption = new BehaviorSubject<object>({});
  ...
  get getUpdatedRedemption() {
    return this.updatedRedemption.asObservable();
  }
  ...
  setUpdatedRedemption(data: any) {
    this.updatedRedemption.next(data);
  }

  getRewardCatalogFromTango(): Observable<any> {
    const basicToken =
      'UUFQbGF0Zm9ybTI6YXBZUGZUNkhOT05wRFJVajNDTEdXWXQ3Z3ZJSE9OcERSVVlQZlQ2SGo=';
    const headers = new HttpHeaders({
      authorization: `Basic ${basicToken}`,
      accept: 'application/json',
    });
    return this.http.get(
      'https://integration-api.tangocard.com/raas/v2/catalogs',
      { headers }
    );
  }
